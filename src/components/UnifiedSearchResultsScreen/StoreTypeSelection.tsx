...
import { keywordCategories, getKeywordLabel } from '../../utils/keywordOptions';
...
const StoreTypeSelection: React.FC<StoreTypeSelectionProps> = ({
  ...
  const toggleCategoryKeywords = (categoryKeywords: { value: string }[], e: React.MouseEvent) => {
    e.stopPropagation();
    const categoryValues = categoryKeywords.map(k => k.value);
    const allSelected = categoryValues.every(value => selectedKeywords.includes(value));
    
    if (allSelected) {
      setSelectedKeywords(prev => prev.filter(k => !categoryValues.includes(k)));
    } else {
      setSelectedKeywords(prev => {
    ...
    <div className="mb-6">
      ...
      <button
        ...
        className={`mb-2 px-3 py-1 rounded-full text-sm font-medium transition-colors duration-200
          ${selectedKeywords.length === keywordCategories.flatMap(c => c.keywords).length
            ? 'bg-primary-500 text-white'
            : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
      >
        {selectedKeywords.length === keywordCategories.flatMap(c => c.keywords).length ? '全解除' : '全選択'}
      </button>

      <div className="space-y-2">
        {keywordCategories.map((category) => {
          const categoryKeywords = category.keywords.map(k => k.value);
          const selectedCount = categoryKeywords.filter(k => selectedKeywords.includes(k)).length;
          const isPartiallySelected = selectedCount > 0 && selectedCount < categoryKeywords.length;
              ...
              <div className="flex items-center bg-gray-50">
                ...
                >
                  <div className={`w-4 h-4 rounded border flex items-center justify-center
                    ${isFullySelected ? 'bg-primary-500 border-primary-500' : 
                      isPartiallySelected ? 'bg-primary-200 border-primary-500' : 
                      'border-gray-300'}`}
                  >
                    {isFullySelected && (
      ...
      {customKeywords.length > 0 && (
        <div className="mt-4">
          <h4 className="text-sm font-medium text-gray-700 mb-2">カスタム検索タイプ</h4>
          <div className="flex flex-wrap gap-2">
