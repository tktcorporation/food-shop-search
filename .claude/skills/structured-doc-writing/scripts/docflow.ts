...
function approvalIsCurrent(project: string, state: State, approval: Approval): boolean {
  ...
  return state.approvals[approval]
    && existsSync(file)
...
function assertReadyForApproval(project: string, state: State, approval: Approval): void {
  ...
  if (approval === "outline" && (!content.includes("情報の序列：") || !content.includes("表示方法："))) {
    fail("outline.md must define information hierarchy and presentation format");
  }
  if (approval === "review" && (!content.includes("情報の強弱とつながり") || !content.includes("段落・箇条書き・表の使い分け"))) {
    fail("review.md must check information hierarchy and presentation format");
  }
}

function init(project: string, title: string): void {
  if (existsSync(statePath(project))) fail(`already initialized: ${project}`);
  ...
  writeNew(resolve(project, "brief.md"), `# ${title}：目的定義\n\n## 読者\n\n${PLACEHOLDER}\n\n## 読後の状態\n\n${PLACEHOLDER}\n\n## 文書の問い\n\n${PLACEHOLDER}\n\n## 対象範囲\n\n${PLACEHOLDER}\n\n## 対象外\n\n${PLACEHOLDER}\n\n## 制約\n\n${PLACEHOLDER}\n`);
  writeNew(resolve(project, "evidence.md"), `# ${title}：根拠\n\n| ID | 事実 | 出典 | 確実性 | 備考 |\n|---|---|---|---|---|\n| E-001 | ${PLACEHOLDER} | ${PLACEHOLDER} | ${PLACEHOLDER} | |\n\n## 計算結果\n\n| ID | 結果 | 入力となる根拠 | 式 |\n|---|---|---|---|\n| C-001 | ${PLACEHOLDER} | ${PLACEHOLDER} | ${PLACEHOLDER} |\n`);
  writeNew(resolve(project, "outline.md"), `# ${title}：構成\n\n## section-id：章名\n\n読者の問い：${PLACEHOLDER}\n\nこの章の結論：${PLACEHOLDER}\n\n情報の序列：\n- 最重要：${PLACEHOLDER}\n- それを支える情報：${PLACEHOLDER}\n- 補足：${PLACEHOLDER}\n\n表示方法：${PLACEHOLDER}\n\n使用する根拠：\n- ${PLACEHOLDER}\n\nこの章に書かないこと：${PLACEHOLDER}\n\n前後の章との関係：${PLACEHOLDER}\n`);
  writeNew(resolve(project, "review.md"), `# ${title}：レビュー\n\n## 構造\n\n${PLACEHOLDER}\n\n### 情報の強弱とつながり\n\n${PLACEHOLDER}\n\n## 根拠\n\n${PLACEHOLDER}\n\n## 可読性\n\n${PLACEHOLDER}\n\n### 段落・箇条書き・表の使い分け\n\n${PLACEHOLDER}\n\n## 指摘の採否\n\n${PLACEHOLDER}\n`);
...
function status(project: string): void {
  ...
  for (const approval of APPROVALS) {
    const label = approvalIsCurrent(project, state, approval)
      ? "approved"
      : state.approvals[approval] ? "changed " : "pending ";
...
function finalize(project: string): void {
  const state = loadState(project);
  requireCurrentApproval(project, state, "draft");
  requireCurrentApproval(project, state, "review");
...
function validate(project: string): void {
  ...
  for (const id of ids) {
    if (!existsSync(resolve(project, "sections", `${id}.md`))) errors.push(`missing section file: ${id}.md`);
  }
...
function usage(): never {
  console.error("usage: docflow <init|status|approve|add-section|assemble|finalize|validate> <project-dir> [args]");
  process.exit(1);
}
